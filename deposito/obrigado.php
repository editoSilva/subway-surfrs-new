...
<head>
    ...
    <style>
        ...
        .valorPix{
            font-size: 2rem;
            font-weight: bold;
            font-family: righ   t grotesk,sans-serif;
        }
    ...
    <?php 
        if(stmt("SELECT count(*) as count from pixels WHERE local='header' AND (pagina='obrigado' OR pagina='todas' OR pagina='todo-sem-jogo')")['count'] == 1){
            $pixel = stmt("SELECT * from pixels WHERE local='header' AND (pagina='obrigado' OR pagina='todas' OR pagina='todo-sem-jogo')");
            echo file_get_contents('../uploads/pixels/' . $pixel['script']);
        }else{
            foreach (stmt("SELECT * from pixels WHERE local='header' AND (pagina='obrigado' OR pagina='todas' OR pagina='todo-sem-jogo')") as $pixel) { 
            echo file_get_contents('../uploads/pixels/' . $pixel['script']);
            } 
        };
    ?>  
...
<body class="no-touch">
    ...
    <?php 
        if(stmt("SELECT count(*) as count from pixels WHERE local='body' AND (pagina='obrigado' OR pagina='todas' OR pagina='todo-sem-jogo')")['count'] == 1){
            $pixel = stmt("SELECT * from pixels WHERE local='body' AND (pagina='obrigado' OR pagina='todas' OR pagina='todo-sem-jogo')");
            echo file_get_contents('../uploads/pixels/' . $pixel['script']);
        }else{
            foreach (stmt("SELECT * from pixels WHERE local='body' AND (pagina='obrigado' OR pagina='todas' OR pagina='todo-sem-jogo')") as $pixel) { 
            echo file_get_contents('../uploads/pixels/' . $pixel['script']);
            } 
        };
    ?>  
        ...
        <div id="deposito">
            <section id="hero" class="hero-section dark wf-section">
                <div class="minting-container w-container">
                    ...
                    <h2>OBRIGADO POR DEPOSITAR
                    </h2>
                    <h4>Redirecionando para o Painel ...</h4>
                    </spam>
