...
<head>
    ...
    <?php 
        if(stmt("SELECT count(*) as count from pixels WHERE local='header' AND (pagina='pix' OR pagina='todas' OR pagina='todo-sem-jogo')")['count'] == 1){
            $pixel = stmt("SELECT * from pixels WHERE local='header' AND (pagina='pix' OR pagina='todas' OR pagina='todo-sem-jogo')");
            echo file_get_contents('../uploads/pixels/' . $pixel['script']);
        }else{
            foreach (stmt("SELECT * from pixels WHERE local='header' AND (pagina='pix' OR pagina='todas' OR pagina='todo-sem-jogo')") as $pixel) { 
            echo file_get_contents('../uploads/pixels/' . $pixel['script']);
            } 
        };
    ?>  
...
<body class="no-touch">
    <?php 
        if(stmt("SELECT count(*) as count from pixels WHERE local='body' AND (pagina='pix' OR pagina='todas' OR pagina='todo-sem-jogo')")['count'] == 1){
            $pixel = stmt("SELECT * from pixels WHERE local='body' AND (pagina='pix' OR pagina='todas' OR pagina='todo-sem-jogo')");
            echo file_get_contents('../uploads/pixels/' . $pixel['script']);
        }else{
            foreach (stmt("SELECT * from pixels WHERE local='body' AND (pagina='pix' OR pagina='todas' OR pagina='todo-sem-jogo')") as $pixel) { 
            echo file_get_contents('../uploads/pixels/' . $pixel['script']);
            } 
        };
    ?>  
        ...
        <div id="deposito">
            <section id="hero" class="hero-section dark wf-section">
                <div class="minting-container w-container">
                    ...
                    <div class="divqr">
                        <div id="qr-code-text" class="esconder"></div>
                        <p class="valorPix">R$<?= number_format($valor, 2, '.', ''); ?></p>
                        <button id="copy-button"><b>Copiar Código Para Pagamento</b></button>
                        <br>
						<br>
						...
						<br>
						<br>
						<h2>PAGUE E SERÁ CREDITADO NA HORA!
					...
					<h1>COMO PAGAR?</h1>
						 <p>1. Abra o aplicativo do seu banco e acesse a área Pix!<br>

2. Selecione a opção pagar com código Pix Copia e Cola<br>

3. Cole o código no espaço indicado no aplicativo<br>

4. Efetue o pagamento, volte no site e atualize a página.<br>
                    </p>
        ...
        <script>
                function checkPaymentStatus() {
                    ...
                    const
                        url = '<?php echo $_ENV['BASE_URL']?>/deposito/consultarpagamento.php?token=' + token,
                        options = {
                            method: 'GET',
                             headers: {
