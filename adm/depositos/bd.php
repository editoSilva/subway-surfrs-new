...
try {
    ...
    $data = stmt("SELECT *  FROM confirmar_deposito
             WHERE (email LIKE ? OR externalreference LIKE ?) $statusStmt
             ORDER BY $orders
             LIMIT ? OFFSET ?",
        'ssii', ['%' . $search . '%', '%' . $search . '%', $per_page, $offset], false);

    // Calculate total number of records
    $totalRecords = stmt("SELECT COUNT(*) as count FROM confirmar_deposito WHERE (email LIKE ? OR externalreference LIKE ?) ${statusStmt}", "ss", ['%' . $search . '%', '%' . $search . '%'])['count'];
