...
try {
    ...
    $data = stmt("SELECT *  FROM saque_afiliado
    ...
    $totalRecords = stmt("SELECT COUNT(*) as count FROM saque_afiliado WHERE (email LIKE ? OR pix LIKE ?) ${statusStmt}", "ss", ['%' . $search . '%', '%' . $search . '%'])['count'];
