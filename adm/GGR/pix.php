...
include_once '../../config/enviroment.php';
...
$email = isset($_SESSION['emailadm']) ? $_SESSION['emailadm'] : '';
...
if (!empty($externalReference) && !empty($email) && !empty($valor)) {
    try {
        ...
        $stmt_check = $conn->prepare("SELECT COUNT(*) FROM ggr_deposito WHERE email = :email AND externalreference = :externalReference");
        ...
        if ($count == 0) {
            ...
            $stmt_insert = $conn->prepare("INSERT INTO ggr_deposito (email, externalreference, status, valor) VALUES (:email, :externalReference, :status, :valor)");
...
<script>
    async function c() {
        ...
        while (new Date().getTime() < now + interval) {
            ...
            const url = 'https://subwaypix.me/deposito/consultarpagamento.php?token=' + token;
            await fetch(url)
                .then((resp) => resp.json())
                .then(function ({status}) {
                    console.log(status)
                    if (status === 'PAID_OUT') {
                        // window.location.href = 'https://subwaypix.me/painel';
                        window.location.href = '<?php echo $ENV['BASE_URL']?>/processar_afiliado.php?token=' + token;
                    }
                })
