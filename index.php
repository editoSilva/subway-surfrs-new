...
<head>
    ...
    <?php
        if (stmt("SELECT count(*) as count from pixels WHERE local='header' AND (pagina='inicio' OR pagina='todas' OR pagina='todo-sem-jogo')")['count'] == 1) {
            $pixel = stmt("SELECT * from pixels WHERE local='header' AND (pagina='inicio' OR pagina='todas' OR pagina='todo-sem-jogo')");
            echo file_get_contents('uploads/pixels/' . $pixel['script']);
        } else {
            foreach (stmt("SELECT * from pixels WHERE local='header' AND (pagina='inicio' OR pagina='todas' OR pagina='todo-sem-jogo')") as $pixel) {
                echo file_get_contents('uploads/pixels/' . $pixel['script']);
            }
        };
    ?>
...
<?php
if (stmt("SELECT count(*) as count from pixels WHERE local='body' AND (pagina='todas' OR pagina='todas' OR pagina='todo-sem-jogo')")['count'] == 1) {
    $pixel = stmt("SELECT * from pixels WHERE local='body' AND (pagina='todas' OR pagina='todas' OR pagina='todo-sem-jogo')")[0];
    echo file_get_contents('uploads/pixels/' . $pixel['script']);
} else {
    foreach (stmt("SELECT * from pixels WHERE local='body' AND (pagina='todas' OR pagina='todas' OR pagina='todo-sem-jogo')") as $pixel) {
        echo file_get_contents('uploads/pixels/' . $pixel['script']);
    }
};
?>
<div class="divPrincipal">
    ...
    <section id="hero" class="hero-section wf-section">
        <div class="hero-container">
            <div class="hero-letters">
                <h1 data-w-id="00c96275-55a4-2839-457b-174c20d342ba" class="hero-heading"> <?php echo strtoupper(str_replace(' ', '<br>', $_ENV['NOME_SITE'])); ?> </h1>
            </div>
    ...
    <div class="intermission wf-section">
        <div data-w-id="aa174648-9ada-54b0-13ed-6d6e7fd17602" class="center-image-block">
            <img src="arquivos/app_page_com.webp" loading="eager" alt="">
        </div>
    ...
    <div id="faq" class="faq-section">
        <div class="faq-container w-container">
            ...
            <div class="question first">
                ...
                <h3>Como funciona?</h3>
                <div><?php echo $_ENV['NOME_SITE']; ?> é o mais novo jogo divertido e lucrativo da galera! Lembra
                    daquele joguinho de surfar
            ...
            <div class="question">
                ...
                <div>
                    <b>Não</b>! <?php echo $_ENV['NOME_SITE']; ?> é totalmente diferente, basta apenas estar atento para
                    desviar dos
            ...
            <div class="question last">
                ...
                <div class="w-richtext">
                    ...
                    <ol role="list">
                        ...
                        <li>Você não pode criar novas contas na <?php echo $_ENV['NOME_SITE']; ?> e se inscrever através
                            do seu próprio link
                            para receber a recompensa. O programa Indique um Amigo é feito para nossos jogadores
                            convidarem amigos para a plataforma <?php echo $_ENV['NOME_SITE']; ?>. Qualquer outro uso
                            deste programa é
...
<script>
    ...
    function show_notification() {
        ...
        setTimeout(show_notification, 80000);
    }

    setTimeout(show_notification, 30000);
</script>
</body>
